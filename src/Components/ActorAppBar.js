import { AppBar, Box, Toolbar, IconButton, Container, MenuIcon, jwtDecode, iconAplicari, iconHome, iconListaFilme, iconLogout,
  useState, iconPlanner, iconProfile, useEffect, io ,useNavigate, Button,iconDashboard} 
from './Imports';
...
export default function ActorAppBar() {
  ...
  const pages = [
    { label: 'Acasă', path: '/' },
    { label: 'Lista Filme', path: '/listafilme' },
    { label: 'Aplicări', path: '/aplicări' },
    { label: 'Programări', path: '/programariActori' },
    { label: 'Dashboard', path: '/dashboard'},
    { label: 'Profil', path: '/Actor' },
  ];
  ...
  useEffect(() => {
    if (actorId) {
      ...
      newSocket.emit('authenticateActor', actorId); 
              ...
              <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', display: isSmallScreen ? 'none' : 'flex' }}>
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <img src={iconLogout} alt="logout-icon" style={{ width: '18px', height: '18px' }} />
